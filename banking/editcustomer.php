...
$result=$conn->query("SELECT * FROM `customer` WHERE id=$id") or die(mysql_error());
$rws = mysqli_fetch_array($result);
?>
<?php
                        $delete_id = ($_REQUEST['customer_id']);
                        if(isset($_REQUEST['submit2_id'])){
                            $sql_delete=$conn->query("DELETE FROM `customer` WHERE `id` = '$delete_id'") or die(mysql_error());
                            $sql_drop=$conn->query("DROP TABLE passbook".$delete_id) or die(mysql_error());
