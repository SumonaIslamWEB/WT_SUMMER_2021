...
if($option=='ATM')
    ...
    $result = $conn->query("SELECT * FROM cheque_book WHERE account_no='$account_no'");
    
    $rws=mysqli_fetch_array($result);
    $c_status=$rws[3];
    $c_id=$rws[2];
    
    $result = $conn->query("SELECT * FROM atm WHERE account_no='$account_no'");
...
elseif($option=='ATM'){
$sql = $conn->query("insert into atm values('','$name','$account_no','$atm_status')");
...
$sql = $conn->query("insert into cheque_book values('','$name','$account_no','$cheque_book_status')");
