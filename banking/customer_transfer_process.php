...
<?php
     ...
     $result = $conn->query("SELECT MAX(transactionid) from passbook".$reciever_id);
     $rws=  mysqli_fetch_array($result);
     $r_last_tid=$rws[0];
    
    //select the details in the last row of reciever's passbook.
    $result = $conn->query("SELECT * from passbook".$reciever_id." WHERE transactionid='$r_last_tid'");
    ...
    //select the last transaction id in the sender's passbook
     $result = $conn->query("SELECT MAX(transactionid) from passbook".$sender_id);
     $rws=  mysqli_fetch_array($result);
     $s_last_tid=$rws[0];
    
    //select the details in the last row of sender's passbook.
    $result = $conn->query("SELECT * from passbook".$sender_id." WHERE transactionid='$s_last_tid'");
    while($rws= mysqli_fetch_array($result)) {
    $s_amount=$rws[7];
    $s_name=$rws[2];
    $s_branch=$rws[3];
    ...
    elseif($s_total<500)
    {
        ...
        $result = $conn->query("insert into passbook".$reciever_id." values('','$date','$r_name','$r_branch','$r_ifsc','$t_amount','0','$r_total','BY $s_name')");
        
        //insert statement into sender passbook.
        $s_total=$s_amount-$t_amount;
        $result = $conn->query("insert into passbook".$sender_id." values('','$date','$s_name','$s_branch','$s_ifsc','0','$t_amount','$s_total','TO $r_name')");
