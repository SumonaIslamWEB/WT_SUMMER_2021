...
<div class='content_customer'>
    ...
    <?php 
        ...
        $result = $conn->query("SELECT * FROM cheque_book WHERE account_no='$sender_id'") or die(mysql_error());
        $rws=mysqli_fetch_array($result);
        $c_status=$rws[3];
        $c_id=$rws[2];
        
        $result = $conn->query("SELECT * FROM atm WHERE account_no='$sender_id'") or die(mysql_error());
        $rws=mysqli_fetch_array($result);
        $atm_status=$rws[3];
        $a_id=$rws[2];
